...
- Проверка проходит, в некотором роде, с использованием `api` сайта `ychecker.com`. Обязательно использование `proxy` (и чем больше кол-во самих `proxy`, тем большее кол-во `email'ов` удастся проверить), в противном случае лиимит - 100шт `email'ов`;
